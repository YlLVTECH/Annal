...
- `index.html` holds the entire UI skeleton; frontend logic is modular in `src/` (no framework): `main.ts` wires everything, `editor.ts` (CodeMirror 6), `markdownModel.ts` (block model + marked/DOMPurify), `virtualPreview.ts` (virtualized preview), `outline.ts`, `sidebar.ts`, `history.ts`, `documentPosition.ts`, `i18n.ts` etc. `src/styles.css` defines light/dark themes.
