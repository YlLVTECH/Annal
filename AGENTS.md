...
- `index.html` holds the entire UI skeleton; frontend logic is modular in `src/` (no framework). `main.ts` is a 5-line entrypoint; `app/bootstrap.ts` is the composition root and small coordinators live under `src/app/` (`save.ts`, `notes.ts`, `fileSync.ts`, `settings.ts`, `window.ts`, etc.).
- Global application state uses explicit signals in `state.ts` (implementation in `signal.ts`); consumers subscribe and update UI instead of mutation sites manually calling render functions. Collection signals use immutable replacement semantics (new Array/Set). Low-frequency lifecycle notifications use the typed `events.ts` bus.
- The editing trio has a synchronous one-way hot path assembled in `pipeline.ts`: `editor.ts` invokes an injected callback → `markdownModel.applyEdit` → `virtualPreview` / debounced `outline` / save bookkeeping. Do not route per-keystroke work through the generic event bus (measured regression), and do not add direct editor↔model↔preview imports that bypass this flow.
- `editor.ts` owns CodeMirror 6, `markdownModel.ts` owns the block model + marked/DOMPurify, `virtualPreview.ts` owns virtualized preview DOM, and `src/styles.css` defines light/dark themes. Architecture benchmark artifacts and the before/after report live in `benchmark/`.
...
- Frontend modules of note beyond the editing trio: `state.ts`/`signal.ts` (explicit signals), `events.ts` (typed low-frequency bus), `pipeline.ts` (edit hot-path composition root), `outline.ts`, `sidebar.ts`, `shortcuts.ts`, `history.ts`, `findReplace.ts`, `i18n.ts` with `src/i18n/{zh-CN,en-US}.json`, and `src/styles.css` for light/dark themes.
- Auxiliary directories: `benchmark/` holds performance artifacts, scenarios, and `PERFORMANCE.md`; `scripts/` holds model-test and repro scripts; `showcase/` is a standalone demo; `public/i18n/` contains static i18n assets.
...
- Keystroke hot path is incremental (no O(full-document) work per key): `editor.ts` feeds the model a CodeMirror `Text` (row/range slicing via `DocText`), `markdownModel.ts` `applyEdit` uses a single-block fast path, a block-level splice re-parse for newline edits (verify window edge blocks byte-identical, fall back to full re-parse), and line-number shifting for everything after the edited window. Word count is maintained incrementally per transaction.
- `update_note` (autosave) writes the note file but only marks the index dirty; `index.json` is flushed lazily (1s debounce + on blur/pagehide/close via `flush_index` / `close_ready`). Structural commands (create/delete/rename/pin/restore) still flush eagerly.
- `search_notes` caches note contents keyed by (path, mtime, size) and matches case-insensitively without allocating per-file lowercase copies.
- External file change detection is event-driven: a `notify` watcher on note directories emits `fs-notes-changed`; the frontend reacts (suppressing self-save events) instead of a 3s poll; a 60s fallback poll remains.
- highlight.js is loaded lazily (dynamic import) the first time a code block enters the viewport.
