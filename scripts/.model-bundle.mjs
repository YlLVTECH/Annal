...
// stub:dompurify
var dompurify_default = { sanitize: (html) => html };

// stub:hljs
var hljs_default = { getLanguage: () => false, highlight: () => ({ value: "" }) };
...
var renderBaseDir = "";
function setRenderBaseDir(dir) {
  renderBaseDir = dir;
}
var imgAspectCache = /* @__PURE__ */ new Map();
function getImgAspect(src) {
  return imgAspectCache.get(src);
}
function setImgAspect(src, width, height) {
...
function sanitize(html) {
  return dompurify_default.sanitize(html, SANITIZE_OPTS);
}
function renderSingle(token, links) {
  const list = Object.assign([token], { links });
  return f.parser(list);
}
var SIMPLE_KINDS = /* @__PURE__ */ new Set(["paragraph", "heading", "hr", "code", "table", "html"]);
var STRUCTURAL_LINE_RE = /^\s*(?:$|#{1,6}\s|`{3,}|~{3,}|>\s?|[-*+]\s|\d+[.)]\s|[-*+]\s+\[[ xX]\]\s|[-*_]\s*(?:[-*_]\s*){1,}|=+\s*$|-+\s*$|\[[^\]]+\]:\s|<\w|\|)/;
...
function makeBlock(token, src, startOffset, startLine) {
  const raw = token.raw ?? "";
...
function buildBlocks(tokens, src) {
  const out = [];
  let offset = 0;
  let line = 0;
...
function estimateHeight(b2) {
  const lines = Math.max(1, b2.endLine - b2.startLine + 1);
  switch (b2.type) {
    case "code":
      return Math.max(30, lines * 21 + 30);
    case "heading":
      return b2.raw.startsWith("# ") || b2.raw.startsWith("## ") ? 46 : 38;
    case "hr":
      return 26;
    case "table":
...
function assignEstimatedHeights(list) {
  for (const b2 of list) b2.height = estimateHeight(b2);
}
...
function applyEdit(text, range, opts) {
  ...
  if (blocks.length === 0) {
    fullParse(text);
    for (const b2 of blocks) changed.add(b2.id);
    return changed;
  }
  let lo = 0;
  ...
  if (first === lastAffected && opts?.newlineChange !== true) {
    const b2 = blocks[first];
    let structural = false;
    for (let ln2 = range.start; ln2 <= range.end; ln2++) {
      const lineText = getLine(text, ln2);
      if (isStructuralLine(lineText)) {
        structural = true;
        break;
      }
    }
    if (b2.simple && !structural) {
      let slice = text.slice(offsetOfLine(text, b2.startLine), offsetOfLine(text, b2.endLine + 1));
      ...
      if (single && single.type === b2.type && (single.raw ?? slice) === slice) {
        updateSingleBlock(b2, slice);
        b2._token = single;
        changed.add(b2.id);
        return changed;
      }
  ...
  for (const b2 of blocks) previous.set(b2.key, b2);
  const oldOrder = blocks;
  fullParse(text);
  const newById = /* @__PURE__ */ new Map();
  for (const b2 of blocks) {
    const old = previous.get(b2.key);
    if (old) {
      b2.id = old.id;
      b2.version = old.version;
      b2.html = old.html;
      b2.hlDone = old.hlDone;
      b2.height = old.height;
      b2._token = old._token && old.key === b2.key ? old._token : void 0;
    } else {
      changed.add(b2.id);
    }
    newById.set(b2.id, b2);
  }
...
function updateSingleBlock(b2, newRaw) {
  const newLines = countNewlines(newRaw);
  const oldLines = b2.endLine - b2.startLine;
  b2.raw = newRaw;
  b2.key = hashText(newRaw);
  b2.version++;
  b2.html = "";
  b2.hlDone = false;
  b2.hasImage = /!\[[^\]]*\]\([^)]*\)/.test(newRaw);
  b2.endLine = b2.startLine + newLines;
  if (newLines !== oldLines || b2.height === 0) {
    b2.height = estimateHeight(b2);
  }
  b2._token = void 0;
}
...
function offsetOfLine(text, lineNo) {
  let line = 0;
  let i = 0;
...
function renderBlockHtml(b2) {
  if (b2.html) return b2.html;
  try {
    const token = blockToken(b2);
    const links = token.links ?? docLinks;
    b2.html = renderSingle(token, links);
  } catch {
    b2.html = sanitize(`<p>${escapeHtml(b2.raw)}</p>`);
  }
  return b2.html;
}
function blockToken(b2) {
  if (b2._token) return b2._token;
  try {
    const res = lexSnippet(b2.raw, docLinks);
    for (const t of res) {
      if (t.type === "space" || t.type === "def") continue;
      b2._token = t;
      return t;
...
function highlightBlock(b2) {
  if (b2.hlDone || !b2.isCode) return;
  const token = blockToken(b2);
  const language = (token.lang ?? "").split(/\s+/)[0];
  const text = token.text ?? b2.raw;
  let body;
  if (language && hljs_default.getLanguage(language)) {
    body = hljs_default.highlight(text, { language, ignoreIllegals: true }).value;
  } else {
    body = escapeHtml(text);
  }
  const cls = language ? ` class="hljs language-${escapeAttr(language)}"` : ' class="hljs"';
  b2.html = sanitize(`<pre><code${cls}>${body}</code></pre>`);
  b2.hlDone = true;
}
function renderMarkdownWhole(src, baseDir = "") {
